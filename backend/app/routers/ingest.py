...
async def _resolve_ingest_source(
...
) -> tuple[SourceInfo, dict, list[str]]:
    ...
    video_ids = await resolve_source_video_ids(source_info)
    source_meta = await fetch_source_metadata(source_info, video_ids)
    
    return source_info, source_meta, video_ids
...
async def ingest_source(
    ...
    try:
        source_info, source_meta, video_ids = await _resolve_ingest_source(request)
        
        if len(video_ids) > 100 and not request.limit_override:
            video_ids = video_ids[:100]

        existing_source = await get_source_with_videos(db, source_info.source_id)
        ...
        metadata_by_video = await fetch_videos_metadata(videos_to_process)

        for video_id in videos_to_process:
            video_meta = metadata_by_video.get(video_id, {})
            try:
                ...
                chunks = chunk_transcript(segments, video_id, source_info.source_id, video_meta)
        ...
        await mark_source_status(db, source_info.source_id, status)
